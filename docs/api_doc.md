...
- **src/cgdf/graphics/core/**
  ...
  - [material.h](#api-src-cgdf-graphics-core-material-h)
  - [mesh.h](#api-src-cgdf-graphics-core-mesh-h)
  - [model.h](#api-src-cgdf-graphics-core-model-h)
  - [objloader.h](#api-src-cgdf-graphics-core-objloader-h)
...
- **src/cgdf/graphics/opengl/shaders/**
  - [default_shader.h](#api-src-cgdf-graphics-opengl-shaders-default-shader-h)
  - [light2d_shader.h](#api-src-cgdf-graphics-opengl-shaders-light2d-shader-h)
  - [model_shader.h](#api-src-cgdf-graphics-opengl-shaders-model-shader-h)
  - [spritebatch_shader.h](#api-src-cgdf-graphics-opengl-shaders-spritebatch-shader-h)
...
- ### array.h:
  ...
  **Функции:**</br>
  ...
  - Получение элемента по индексу (адрес ячейки в памяти):</br>
  ...
  - Найти элемент и вернуть его индекс + 1 (счет не с нуля). 0 = нет такого элемента:</br>
...
- ### files.h:
  > Описание: Реализует работу с файлами.
  ...
  **Функции:**</br>
  ...
  - Получить директорию файла (требуется освободить память):</br>
    `char* Files_dirname_dup(const char *filepath);`

  - Склеить пути (требуется освободить память):</br>
...
<a id="api-src-cgdf-core-info-h"></a>
- ### info.h:
  > Описание: Определяет функции для получения информации о CPU и RAM.
  ...
  **Типы данных:**</br>
  typedef `CpuInfo`:
  - Информация о процессоре.
  - Объявление: `typedef struct CpuInfo CpuInfo;`

  typedef `MemInfo`:
  - Информация о памяти.
  - Объявление: `typedef struct MemInfo MemInfo;`

  **Функции:**</br>
  - Получить архитектуру процессора в виде строки:</br>
    `const char* Info_get_cpu_arch_name(Info_cpu_arch arch);`

  - Функция для получения информации о процессоре:</br>
    `CpuInfo Info_get_cpu();`

  - Функция для получения ОЗУ (в байтах):</br>
    `MemInfo Info_get_mem();`

  **Глобальные переменные:**</br>
  `g_Info_cpu_cached_`:
  - Объявление: `extern bool g_Info_cpu_cached_;`.

  `g_Info_cpu_info_cache_`:
  - Объявление: `extern CpuInfo g_Info_cpu_info_cache_;`.


<a id="api-src-cgdf-core-logger-h"></a>
- ### logger.h:
  > Описание: Логгер событий.
...
- ### mm.h:
  ...
  **Функции:**</br>
  ...
  - Выделение памяти с явным выравниванием:</br>
...
<a id="api-src-cgdf-core-node-h"></a>
- ### node.h:
  > Описание: Определяет функционал системы дерева из узлов. Работает на основе ленивого дерева.

  [Назад](#content)

  **Определения:**</br>
  `NODE_DEFAULT_CHILDREN_COUNT`:
  - Количество дочерних узлов по умолчанию.
  - Значение: `32`.
  ...
  **Типы данных:**</br>
  typedef `Node`:
  - Структура нода.
  - Объявление: `typedef struct Node Node;`
...
- ### pixmap.h:
  ...
  **Глобальные переменные:**</br>
  `g_Pixmap_default_icon`:
  - Определяем глобальные переменные стандартной картинки.
  - Объявление: `extern const unsigned char g_Pixmap_default_icon[];`.

  `g_Pixmap_default_icon_size`:
  - Объявление: `extern const size_t g_Pixmap_default_icon_size;`.

  `g_Pixmap_default_icon_width`:
  - Объявление: `extern const int g_Pixmap_default_icon_width;`.

  `g_Pixmap_default_icon_height`:
  - Объявление: `extern const int g_Pixmap_default_icon_height;`.
...
- ### vector.h:
  ...
  **Функции:**</br>
  - Функция `Vec2i_len`:</br>
    `static inline int Vec2i_len(Vec2i v);`
  ...
  - Функция `Vec3i_len`:</br>
    `static inline int Vec3i_len(Vec3i v);`
  ...
  - Функция `Vec4i_len`:</br>
    `static inline int Vec4i_len(Vec4i v);`
  ...
  - Функция `Vec2f_norm`:</br>
    `static inline Vec2f Vec2f_norm(Vec2f v);`

  - Функция `Vec3f_len`:</br>
  ...
  - Функция `Vec3f_norm`:</br>
    `static inline Vec3f Vec3f_norm(Vec3f v);`

  - Функция `Vec4f_len`:</br>
    `static inline float Vec4f_len(Vec4f v);`
  ...
  - Функция `Vec4f_norm`:</br>
    `static inline Vec4f Vec4f_norm(Vec4f v);`

  - Функция `Vec2d_len`:</br>
    `static inline double Vec2d_len(Vec2d v);`
  ...
  - Функция `Vec2d_norm`:</br>
    `static inline Vec2d Vec2d_norm(Vec2d v);`

  - Функция `Vec3d_len`:</br>
    `static inline double Vec3d_len(Vec3d v);`
  ...
  - Функция `Vec3d_norm`:</br>
    `static inline Vec3d Vec3d_norm(Vec3d v);`

  - Функция `Vec4d_len`:</br>
    `static inline double Vec4d_len(Vec4d v);`
...
<a id="api-src-cgdf-graphics-core-material-h"></a>
- ### material.h:
  > Описание: Определяет структуру материала.
  ...
  **Типы данных:**</br>
  typedef `Material`:
  - Структура материала.
  - Объявление: `typedef struct Material Material;`

  **Функции:**</br>
  - Создать материал:</br>
    `Material* Material_create( const char *name, Vec4f albedo, Vec3f ambient, float metallic, Vec3f specular, float specular_strength, float roughness, float ao, float normal_strength, Vec3f emissive_color, float emissive_strength, float alpha_cutoff, bool double_sided, bool transparent, float distortion_strength, float distortion_aberration_strength, Texture *albedo_map, Texture *normal_map, Texture *occlusion_map, Texture *roughness_map, Texture *metallic_map, Texture *emissive_map, Texture *height_map );`

  - Создать материал по умолчанию (пустой):</br>
    `Material* Material_create_default(const char *name);`

  - Уничтожить материал:</br>
...
- ### mesh.h:
  ...
  **Функции:**</br>
  - Создать сетку:</br>
    `Mesh* Mesh_create( const Vertex* vertices, uint32_t vertex_count, const uint32_t* indices, uint32_t index_count, bool is_dynamic, Material *material );`

  - Уничтожить сетку:</br>
    `void Mesh_destroy(Mesh **mesh);`

  - Получить материал из сетки:</br>
...
<a id="api-src-cgdf-graphics-core-model-h"></a>
- ### model.h:
  > Описание: Определяет модель. Модель содержит сетки, материалы, шейдеры, нод, и прочее.

  [Назад](#content)

  **Определения:**</br>
  `MODEL_DEFAULT_MESHES_COUNT`:
  - Количество сеток по умолчанию.
  - Значение: `16`.
  ...
  **Типы данных:**</br>
  typedef `Model`:
  - Модель.
  - Объявление: `typedef struct Model Model;`

  **Функции:**</br>
  - Создать модель:</br>
    `Model* Model_create(Renderer *renderer);`

  - Уничтожить модель:</br>
    `void Model_destroy(Model **model);`

  - Добавить сетку в модель:</br>
    `void Model_add_mesh(Model *self, Mesh *mesh);`

  - Удалить сетку из модели:</br>
    `void Model_remove_mesh(Model *self, Mesh *mesh);`

  - Удалить и освободить память сетки из модели:</br>
    `void Model_delete_mesh(Model *self, Mesh *mesh);`

  - Отрисовать модель:</br>
    `void Model_render(Model *self, bool wireframe);`


<a id="api-src-cgdf-graphics-core-objloader-h"></a>
- ### objloader.h:
  > Описание: Определяет функции загрузчика моделей в формате OBJ.
  ...
  **Типы данных:**</br>
  typedef `OBJFile`:
  - Структура объектного файла.
  - Объявление: `typedef struct OBJFile OBJFile;`

  **Функции:**</br>
  - Загрузить модели из OBJ-файла:</br>
    `OBJFile ObjLoader_load(Renderer *renderer, const char *filepath);`


<a id="api-src-cgdf-graphics-core-renderer-h"></a>
- ### renderer.h:
  > Описание: Создаёт общий апи для работы с рендерингом графики.
...
- ### utils.h:
  ...
  **Функции:**</br>
  - Перевести координаты на экране, в мировые координаты в 2D пространстве:</br>
    `static inline Vec2d screen_to_world2d(Camera2D *camera, Vec2d point);`

  - Перевести мировые координаты в 2D пространстве, в пространство экрана:</br>
    `static inline Vec2d world2d_to_screen(Camera2D *camera, Vec2d world_point);`

  - Перевести мировые координаты в 3D пространстве, в пространство экрана:</br>
...
- ### controllers.h:
  ...
  **Типы данных:**</br>
  ...
  typedef `CameraPlanetController3D`:
  - Планетарный контроллер для 3D камеры.
  - Объявление: `typedef struct CameraPlanetController3D CameraPlanetController3D;`
  ...
  **Функции:**</br>
  ...
  - Создать орбитальный 3D контроллер:</br>
    `CameraOrbitController3D* CameraOrbitController3D_create( Window *window, Camera3D *camera, Vec3d target_pos, float mouse_sensitivity, float distance, float friction );`
  ...
  - Создать 3D контроллер:</br>
    `CameraPlanetController3D* CameraPlanetController3D_create( Window *window, Camera3D *camera, float mouse_sensitivity, float ctrl_speed, float speed, float shift_speed, float friction, bool up_is_forward );`

  - Уничтожить 3D планетарный контроллер:</br>
  ...
  **Глобальные переменные:**</br>
  `static const int DEFAULT_MOUSE_POS_OFFSET = 16;`:
  - Константы.
  - Объявление: `static const int DEFAULT_MOUSE_POS_OFFSET = 16;`.
...
- ### buffer_gc.h:
  ...
  **Глобальные переменные:**</br>
  ...
  - Объявление: `extern BufferGC_GL g_buffer_gc_gl;`.
...
- ### texunit.h:
  ...
  **Глобальные переменные:**</br>
  `g_texunits_gl`:
  - Создаём единую глобальную структуру.
  - Объявление: `extern TextureUnits g_texunits_gl;`.
...
<a id="api-src-cgdf-graphics-opengl-shaders-default-shader-h"></a>
- ### default_shader.h:
  > Описание: Шейдер по умолчанию (затычка).
...
<a id="api-src-cgdf-graphics-opengl-shaders-light2d-shader-h"></a>
- ### light2d_shader.h:
  > Описание: Шейдеры 2D освещения.

  [Назад](#content)

  **Функции:**</br>
  - Функция `layout`:</br>
    `static const char* LIGHT2D_SHADER_VERT = "\ #version 330 core\n\ layout (location = 0) in vec3 a_position;\n\`
...
<a id="api-src-cgdf-graphics-opengl-shaders-model-shader-h"></a>
- ### model_shader.h:
  > Описание: Шейдеры модели.
...
<a id="api-src-cgdf-graphics-opengl-shaders-spritebatch-shader-h"></a>
- ### spritebatch_shader.h:
  > Описание: Шейдеры пакетной отрисовки спрайтов.
