//
// spritebatch.h - Определяет API для пакетной отрисовки спрайтов.
//
// Пакетная отрисовка спрайтов работает концептуально также, как в LibGDX.
//
...
extern uint32_t g_BATCH_SPRITES_SIZE;  // Глобальная переменная (редактируется. По умолчанию равно BATCH_MAX_SPRITES).
