...
int main(int argc, char *argv[]) {
    ...
    // Второй параметр (false) означает выводить ли отладочную
    // служебную информацию рендерера (например OpenGL или Vulkan).
    // Наример, если включить то будет выводиться в лог данные по
    // типу какая видеокарта используется, версия граф.библиотеки и тд.
    // Если надо, эти данные можно получить отдельно через спец.функции.
    if (!Window_open(window, false)) {
